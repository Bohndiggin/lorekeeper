...
function writeTable(response, endpoint){
    ...
    for (let i = 0;i<response.data.length;i++){
        ...
        let currentTable = endpoint
        buttonTemp.onclick = ev => {
            sendOneSignal(`${endpoint}/?id=${values[0]}`, currentTable)
        }
    }
}
function writeOne(data, currentTable) {
    ...
    for (const property in related) {
        ...
        addRelatedPropertyTemp.onclick = ev => {
            openAddingWindow(property, currentTable)
        }
    }
    // console.log(relatedValues)
}
function sendOneSignal(endpoint, currentTable) {
    const dmdmsconn = "http://127.0.0.1:8000";
    console.log(endpoint)
    axios.get(dmdmsconn + endpoint)
        .then((response) => writeOne(response.data, currentTable))
...
function populateDropDown(property, currentTable, currentWork) { //Function queries for the proper names for everything in a table to populate dropdown menus
    console.log(currentTable);
    const dmdmsconn = "http://127.0.0.1:8000";
    endpoint = relationMap[currentTable][property]
    axios.get(dmdmsconn + endpoint)
        .then((response) => {
...
function openAddingWindow(property, currentTable) {
    let popUpWindow = document.getElementById('popup')
    popUpWindow.innerHTML = ``
    let popUpWindowBuild = `<div class="popup">
    ...
    populateDropDown(property, currentTable, popUpWindowBuild)
}